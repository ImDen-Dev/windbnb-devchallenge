...
export class ApartmentsService {
  ...
  newStays = new Subject<any>();

  constructor() {}

  getUniqCities(): { city: string; country: string }[] {
    const cities: { city: string; country: string }[] = [];
    this.stays.filter((a: Apartment) => {
      if (!cities.some((y) => a.city === y.city)) {
        cities.push({ city: a.city, country: a.country });
      }
    });
    return cities;
  }
  ...
  filterApartments(location: string, guests: string): void {
    ...
    this.newStays.next(newStays);
  }

  getStays(): Apartment[] {
    return this.stays.slice();
  }
}
